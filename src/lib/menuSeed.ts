...
// Single source of truth for the seeded menus + events. Imported by the build
// seed script (prisma/seed.ts) and the /api/reseed endpoint.
...
export const MENUS: Record<string, Section[]> = {
  lunch: [
    {
...
/** DESTRUCTIVE: replaces all menu items + events with the seed data above. */
export async function reseed(prisma: PrismaClient): Promise<{ menuItems: number; events: number }> {
  ...
  return { menuItems, events: EVENTS.length };
}
